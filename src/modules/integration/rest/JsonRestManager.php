...
class JsonRestManager
{
    ...
    public function addVersion(string $title, string $version_key): bool {
        ...
        return !in_array(file_put_contents($this->version_storage, Yaml::dump($this->rest_versions)), [0, false], true);
    }

    public function getVersion(string $version_key): ?array
    {
        return $this->rest_versions[$version_key] ?? null;
    }
    ...
    public function addVersionRoute(string $key, array $data): bool
    {
        ...
        return !in_array(file_put_contents($this->version_routes_storage, Yaml::dump($this->version_routes,Yaml::DUMP_OBJECT_AS_MAP)), [0, false], true);
    }

    public function getVersionRoute(string $version_key): ?array {

        $found = array_filter($this->version_routes, fn(array $item): bool => isset($item['route_type']) && $item['route_type'] === "rest_". $version_key);
    ...
    public function addVersionRouteDataSourceSetting(string $route_key, string $handler): bool
    {
        ...
        return !in_array(file_put_contents($this->version_storage, Yaml::dump($post_settings, Yaml::DUMP_OBJECT_AS_MAP)), [0, false], true);
    }
