...
trait StaticHelperTrait
{
    ...
    public static function filters(string $table_name, int $limit = 10): array
    {
        ...
        $filters['offset'] = $limit > 0 && $filters['total_rows'] > $limit ? $filters['total_rows'] / $limit : 0;
    ...
    {
        $query = sprintf('SELECT * FROM `%s` ORDER BY `%s` %s LIMIT :limit OFFSET :offset', $table_name, $sort_by, $order_by);
        if ($search_q !== null && $search_q !== '' && $search_q !== '0') {

            if ($search_type == 1) {
                $search_q = sprintf('%%%s%%', $search_q);
            }
         ...
         $query = sprintf("SELECT * FROM `%s` WHERE %s LIKE '%%%s%%' ORDER BY `%s` %s LIMIT :limit OFFSET :offset", $table_name, $search_field, $search_q, $sort_by, $order_by);
        }
    ...
    public static function parseFilter(string $handle_class, string $table_name, array $filters, Request $request, string $mapper_class = StdClass::class): mixed
    {
        ...
        return array_map(fn($item): object => new $mapper_class(...$item), $results);
    }
    ...
    public static function dataDeletion(string $table_name, string $by_column, mixed $by_value): bool
    {
        ...
        if (Session::init()->get('system.deletion.confirmation') === 'yes') {
            $query = sprintf('DELETE FROM `%s` WHERE `%s` = :by_value', $table_name, $by_column);
            $query = Database::database()->con()->prepare($query);
    ...
    public static function checkPasswordStrength($password): string
    {
        ...
        if (preg_match('/[\W]/', (string) $password)) {
            $score++;
        }
